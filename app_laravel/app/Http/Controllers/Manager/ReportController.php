...
class ReportController extends Controller
{
    ...
    public function generateReport(Request $request)
    {
        try {
            ...
            Log::info('ReportController: Starting report generation', [
            ...
            // Get the user's role-based access restrictions
            $userId = Auth::id();
            $userRole = DB::table('users')
                ->where('id', $userId)
                ->value('role_id');

            Log::info('Report generation - User Role ID: ' . $userRole);
            ...
            Log::info('Managed outlet IDs for reports: ' . json_encode($managedOutletIds));

            // If no outlets found, return empty data
            if ($managedOutletIds->isEmpty()) {
                Log::info('No managed outlets found for user role ' . $userRole);
                return response()->json([
                    'noData' => true,
                    'message' => 'No outlets available for your account.'
                ]);
            }

            // Base query for audits
            $query = Audit::with(['outlet', 'user', 'status', 'complianceRequirement'])
                ->whereHas('outlet', function($q) use ($managedOutletIds) {
                    $q->whereIn('id', $managedOutletIds);
                })
            ...
            $grouped = $audits->groupBy(function ($audit) {
                return Carbon::parse($audit->start_time)->format('Y-m-d') . '|' . ($audit->outlet->name ?? 'Unknown');
            });

            $tableRows = [];
            $trendData = [];
            foreach ($grouped as $key => $group) {
                [$date, $outletName] = explode('|', $key);
                $total = $group->count();
